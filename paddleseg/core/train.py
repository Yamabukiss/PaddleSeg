...
def train(model,
    ...
    batch_sampler = paddle.io.DistributedBatchSampler(
        train_dataset, batch_size=batch_size, shuffle=True, drop_last=True) #batch subset of the whole Dataloader
    ...
    avg_loss = 0.0
    avg_loss_list = []
    iters_per_epoch = len(batch_sampler) # 11200(whole data) / batch_size = iters_per_epoch
    best_mean_iou = -1.0
    best_model_iter = -1
    ...
    iter = start_iter
    # different from pytorch is paddle evaluate and save every iter(iter present every batch)
    while iter < iters_per_epoch*iters:
            ...
            if (iter) % log_iters == 0 and local_rank == 0:
                ...
                logger.info(
                    "[TRAIN] epoch: {}, iter: {}/{}, loss: {:.4f}, lr: {:.6f}, batch_cost: {:.4f}, reader_cost: {:.5f}, ips: {:.4f} samples/sec | ETA {}"
                        .format(iter
                                // iters_per_epoch + 1, iter, iters*iters_per_epoch, avg_loss,
        ...
        # here start save model and optimizer
        '''if (iter % save_interval == 0 or iter == iters) and local_rank == 0:'''
        # Each epoch is now saved
        if  local_rank == 0:
